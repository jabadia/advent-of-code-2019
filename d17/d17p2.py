...
def find_robot(camera):
    for y, line in enumerate(camera):
...
def is_scaffold(camera, pos):
    x, y = pos
    if 0 <= x < len(camera[0]) and 0 <= y < len(camera):
        return camera[y][x] == '#'
    else:
        return False


def calculate_route(camera):
    # find robot
    robot, direction = find_robot(camera)
    commands = []
    while True:
        next = move(robot, direction)
        if is_scaffold(camera, next):
            if commands[-1] == 'L' or commands[-1] == 'R':
                commands.append(1)
            else:
                commands[-1] += 1
            robot = next
        elif is_scaffold(camera, move(robot, turn(direction, 'L'))):
            commands.append('L')
            direction = turn(direction, 'L')
        elif is_scaffold(camera, move(robot, turn(direction, 'R'))):
            commands.append('R')
...
def solve(input):
    ...
    while True:
        ...
        camera += chr(int(code))
    print(camera)
    route = calculate_route(camera.strip().split('\n'))
    sequence, functions = segment_routes(route)
    print(''.join(route))
    intcode = Intcode(input)
    intcode.memory[0] = 2

    robot_instructions = [ord(c) for c in '\n'.join([sequence] + list(functions.values()) + ['n', '\n'])]
    # for line in robot_instructions:
    #     for c in line:
    #         intcode.run(ord(c))
    #     intcode.run(10)  # newline

    output = ''
    code = None
    while True:
        last_code = code
        code = intcode.run(robot_instructions)
        if code == 'END':
            print(output)
            # print(last_code)
            break
...
if __name__ == '__main__':
    ...
    #     result = intersections(case.case)
    #     case.check(result)
