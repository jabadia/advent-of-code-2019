...
def solve(input, test=True):
    wire1, wire2 = [wire.split(',') for wire in input.strip().split('\n')]
    ...
    for step in wire1:
        ...
        while distance:
            ...
            steps += 1
            if location not in locations1:
                locations1[location] = steps
            distance -= 1
    location = (0, 0)
    steps = 0
    for step in wire2:
        ...
        while distance:
            location = move(location, direction)
            steps += 1
            if location not in locations2:
                locations2[location] = steps
            distance -= 1
    crossings = set(locations1).intersection(set(locations2))
    return min(locations1[crossing] + locations2[crossing] for crossing in crossings)
