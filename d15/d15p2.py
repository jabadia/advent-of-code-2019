...
def dfs(pos, droid, maze):
    for direction in [NORTH, EAST, SOUTH, WEST]:
        new_pos = move(pos, direction)
        if new_pos in maze:
            continue
        result = int(droid.run(direction))
        if result == WALL:
            maze[new_pos] = '#'
        elif result == SPACE or result == TARGET:
            maze[new_pos] = '.' if result == SPACE else 'T'
            dfs(new_pos, droid, maze)
            droid.run(opposite(direction))
...
def print_maze(maze):
    minx = min(coord[0] for coord in maze)
    maxx = max(coord[0] for coord in maze)
    miny = min(coord[1] for coord in maze)
    maxy = max(coord[1] for coord in maze)
...
def dfs2(pos, maze, path):
    ...
    for direction in [NORTH, EAST, SOUTH, WEST]:
        ...
        if new_pos in path:
            continue
        if maze[new_pos] == '.':
            path_len = 1 + dfs2(new_pos, maze, path + [new_pos])
...
def solve(program):
    ...
    dfs(pos, droid, maze)
    print_maze(maze)
    target_pos = [pos for pos, content in maze.items() if content == 'T'][0]
    return dfs2(target_pos, maze, [])
